...
DROP TABLE IF EXISTS employee_sequences CASCADE;
...
DO $$ BEGIN
    CREATE TYPE job_type AS ENUM ('看護師', '臨床検査技師', '看護助手');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
CREATE TABLE employees (
    ...
    employee_number VARCHAR(10) UNIQUE,                       -- 従業員番号 (emp001, emp002, ...)
    password_hash TEXT,                                        -- bcryptハッシュ化パスワード
    password_changed BOOLEAN DEFAULT FALSE,                    -- 初回パスワード変更済みフラグ
...
CREATE INDEX idx_employees_order ON employees(order_index);
...
CREATE TRIGGER update_employee_sequences_updated_at BEFORE UPDATE ON employee_sequences FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE POLICY "Enable read access for all users" ON employee_sequences FOR SELECT USING (true);
CREATE POLICY "Enable insert access for all users" ON employee_sequences FOR INSERT WITH CHECK (true);
CREATE POLICY "Enable update access for all users" ON employee_sequences FOR UPDATE USING (true);
CREATE POLICY "Enable delete access for all users" ON employee_sequences FOR DELETE USING (true);
...
COMMENT ON TABLE employee_sequences IS '従業員番号の自動採番テーブル（単一行）';

COMMENT ON COLUMN employees.employee_number IS '自動生成される従業員番号 (emp001, emp002, ...)';
COMMENT ON COLUMN employees.password_hash IS 'bcryptハッシュ化パスワード';
COMMENT ON COLUMN employees.password_changed IS '初回パスワード変更済みフラグ';
COMMENT ON COLUMN employees.session_token IS '認証用セッショントークン';
COMMENT ON COLUMN employees.is_system_account IS 'システムアカウントフラグ（admin/developer）';
COMMENT ON COLUMN employees.order_index IS '従業員の表示順序';
COMMENT ON COLUMN employees.last_login IS '最終ログイン日時';
